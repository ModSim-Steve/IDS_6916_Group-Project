...
class MilitaryEnvironment(gym.Env):
    ...
    def _execute_engagement_action(self, action: Dict):
        ...
        # Use constrained targeting if available in MARL environment
        constrained_target = target_pos
        if hasattr(self, '_get_observation_for_agent'):
            try:
                # Get unit observation
                observation = self._get_observation_for_agent(unit_id)
                known_enemies = observation['known_enemies']

                # Filter out rows with all zeros (no enemy)
                valid_rows = np.where(np.any(known_enemies != 0, axis=1))[0]
                if len(valid_rows) > 0:
                    valid_enemy_positions = known_enemies[valid_rows]

                    # Find closest visible enemy to the desired target
                    closest_enemy_idx = 0
                    min_distance = float('inf')

                    for i, enemy_pos in enumerate(valid_enemy_positions):
                        if np.all(enemy_pos == 0):  # Skip empty positions
                            continue
                        dist = np.sum((np.array(target_pos) - enemy_pos) ** 2)
                        if dist < min_distance:
                            min_distance = dist
                            closest_enemy_idx = i

                    # Replace target with closest valid enemy
                    constrained_target = tuple(map(int, valid_enemy_positions[closest_enemy_idx]))
            except Exception as e:
                if self.debug_level > 0:
                    print(f"Error in target constraint: {e}, using original target")
                constrained_target = target_pos
...
class MARLMilitaryEnvironment(MilitaryEnvironment):
    ...
    def _create_marl_action_space(self):
        ...
        return spaces.Dict({
            ...
            'movement_params': spaces.Dict({
                ...
                'distance': spaces.Box(low=0, high=10, shape=(1,), dtype=np.int32)  # Reduced from 20 to 10
            }),
    ...
    def _validate_engagement_action(self, unit_id):
        """
        Check if unit can engage (has visible enemies).

        Args:
            unit_id: ID of unit attempting to engage

        Returns:
            Boolean indicating if engagement is valid
        """
        # Get unit observation
        observation = self._get_observation_for_agent(unit_id)
        ...
        known_enemies = observation['known_enemies']
        has_visible_enemies = np.any(known_enemies.sum(axis=1) > 0)

        return has_visible_enemies
    ...
    def _calculate_marl_rewards(self, actions):
        ...
        Ensures all agents receive individual and team rewards appropriately.
    ...
    def _calculate_team_reward(self):
        """
        Calculate team reward with a focus on enemy elimination and team-based objective progress.

        Returns:
            Float representing the team reward component
        """
        total_reward = 0.0

        # 1. Enemy Elimination Reward - Increased to balance with casualty penalty
        ...
        # Significant reward for eliminating enemies (increased from 10.0 to 20.0)
        enemy_reward = enemy_casualties * 20.0
        total_reward += enemy_reward

        # 2. Team-based Objective Progress - Redesigned to reward multiple agents at objective
        if hasattr(self, 'objective') and self.objective:
            try:
        ...
        # 3. Survival Penalty - Kept the same to maintain importance of force preservation
    ...
    def _calculate_team_objective_reward(self):
        """
        Calculate team reward based on how many agents are close to the objective.
        Rewards force concentration at the objective.

        Returns:
            Float representing team objective reward
        """
        ...
        # Count agents within various ranges
        agents_within_50 = 0
        agents_within_30 = 0
        agents_within_20 = 0
        agents_within_10 = 0
        ...
        # Provide scaled rewards based on count and proximity
        reward += agents_within_50 * 0.5  # Base reward for each agent within extended range
        reward += agents_within_30 * 1.0  # Additional for closer agents
        reward += agents_within_20 * 2.0  # More for even closer agents
        reward += agents_within_10 * 3.0  # Maximum for very close agents
        ...
        if agents_within_50 >= significant_force:
            reward += 5.0  # Bonus for having multiple agents within extended range

        if agents_within_30 >= significant_force:
            reward += 10.0  # Bigger bonus for having multiple agents within engagement range

        if agents_within_10 >= significant_force:
            reward += 20.0  # Major bonus for having multiple agents at the objective

        # Log the reward details if debugging is enabled
        if self.debug_level > 0:
            print(
                f"[REWARD] Team Objective: {agents_within_50} agents within 50, {agents_within_30} within 30, {agents_within_20} within 20, {agents_within_10} within 10")
    ...
    def _calculate_agent_reward(self, agent_id, action):
        ...
        now including proximity bonuses and enhanced engagement rewards.
        ...
        try:
            ...
            # 1. Individual proximity bonus for being close to objective
            if hasattr(self, 'objective') and self.objective:
                distance_to_objective = self._calculate_distance(agent_pos, self.objective)
                proximity_bonus = self._calculate_individual_proximity_bonus(distance_to_objective)
                individual_reward += proximity_bonus

            # 2. Reward for engagement when enemies are visible
            if action_type in [1, 2]:  # ENGAGE or SUPPRESS
                ...
                if visible_enemies:
                    ...
                    # NEW: Check for enemy eliminations (partial credit)
                    # We'll need a helper to track who hit what enemies
                    enemy_hits = self._get_agent_hit_enemies(agent_id)
                    for enemy_id in enemy_hits:
                        # If the enemy is now dead, reward the agent who contributed
                        if self.get_unit_property(enemy_id, 'health', 0) <= 0:
                            individual_reward += 15.0  # Major reward for elimination contribution
                else:
                    # Small penalty for engaging with no visible enemies
                    individual_reward -= 0.5

            # 3. Reward for moving toward objective when appropriate
            ...
            individual_reward = max(-2.0, min(5.0, individual_reward))  # Increased upper bound for elimination rewards

            return individual_reward

        except Exception as e:
            if self.debug_level > 0:
                print(f"[DEBUG] Error in agent reward calculation: {e}")
    ...
    def _calculate_individual_proximity_bonus(self, distance_to_objective):
        ...
        This replaces the team-based proximity bonus.

        Args:
            distance_to_objective: Current distance to objective

        Returns:
            Individual proximity bonus value
        """
        # Define distance thresholds and corresponding bonuses
        # Format: (distance_threshold, bonus_value)
        graduated_bonuses = [
            (5, 3.0),  # Within 5 cells: +3.0 reward
            (10, 2.5),  # Within 10 cells: +2.5 reward
            (15, 2.0),  # Within 15 cells: +2.0 reward
            (20, 1.5),  # Within 20 cells: +1.5 reward
            (30, 1.0),  # Within 30 cells: +1.0 reward
            (40, 0.7),  # Within 40 cells: +0.7 reward
            (50, 0.5),  # Within 50 cells: +0.5 reward
            (60, 0.3),  # Within 60 cells: +0.3 reward
        ]
    ...
    def _get_agent_hit_enemies(self, agent_id):
        ...
        # Get hits from combat manager's tracking
        unit_id = self.agent_manager.get_current_unit_id(agent_id)
        if unit_id:
            hit_enemies = self.combat_manager.get_agent_hits(unit_id)
